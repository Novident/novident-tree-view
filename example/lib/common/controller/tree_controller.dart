...
class TreeController extends BaseTreeController {
  ...
  void insertAt(
    ...
    if (root.id == nodeTargetId && !existInRoot) {
      ...
      for (int i = 0; i < root.length; i++) {
        ...
        if (nodeAtRootPoint.id == nodeTargetId) {
          if (nodeAtRootPoint is! NodeContainer) {
            throw InvalidTypeRef(
              message: 'The node [${nodeAtRootPoint.runtimeType}-$nodeTargetId] is not a '
        ...
        } else if (nodeAtRootPoint is NodeContainer && nodeAtRootPoint.isNotEmpty) {
          bool inserted = insertNodeInSubContainer(nodeAtRootPoint, node, nodeTargetId);
          if (inserted) {
            return;
          }
        }
  ...
  void insertAtRoot(Node node, {bool removeIfNeeded = false}) {
    if (removeIfNeeded) removeAt(node.id, verifyDuplicates: true, ignoreNotify: true);
  ...
  void insertAbove(
    ...
    for (int i = 0; i < root.length; i++) {
      ...
      if (belowNode.details.id == childBelowId) {
        ...
        bool inserted = insertAboveNodeInSubComposite(belowNode, node, childBelowId);
        if (inserted) break;
      }
    }
  }
  ...
  void insertAtWithCallback(String? parentId, Node Function() callback,
    ...
    Node node = callback();
    if (removeIfNeeded) {
      removeWhere((Node element) => element.id == node.id, verifyDuplicates: true);
    }
    if (insertInRoot) {
    ...
    for (int i = 0; i < root.length; i++) {
      ...
      if (nodeAtRootPoint is NodeContainer) {
        if (nodeAtRootPoint.id == parentId) {
          ...
          bool inserted =
              insertAtInSubCompositeWithCallback(nodeAtRootPoint, callback, parentId);
          if (inserted) {
  ...
  bool expandAllUntilTarget(Node targetNode, {bool openTargetIfNeeded = false}) {
    ...
    Future<bool> expandNodeWhen(List<Node> tree, {bool ignoreBySubNode = false}) async {
      ...
      for (int i = 0; i < tree.length; i++) {
        ...
        if (node is NodeContainer && node.id == targetNode.id && openTargetIfNeeded) {
        ...
        if (node is NodeContainer && node.isNotEmpty) {
          bool founded = await expandNodeWhen(node.children, ignoreBySubNode: true);
          if (founded) {
            return true;
          }
        }
  ...
  bool removeChild(
    ...
    if (target == null && predicate == null) {
      throw const InvalidOperation(
          message: 'target param and predicate cannot be null at the same time. '
