...
import '../interfaces/tree_common_ops.dart';
...
class TreeController extends ChangeNotifier implements TreeOperations {
  _Root _root = _Root(
    node: Node.withId(-1),
    children: List.from([]),
  ...
  final NodeSelection _selection = NodeSelection(selection: []);

  TreeController(String rootId, List<TreeNode> nodes) {
    _root = _root.copyWith(children: [...nodes]);
    setNewIdToRoot(rootId);
  }

  Stream<TreeStateChanges> get changes => _root.changes;
  ...
  /// of the tree
  List<TreeNode> get root {
    verifyState();
    return [..._root.children];
  }

  set root(List<TreeNode> newRoot) {
    verifyState();
    _root.addNewChange([...newRoot], TreeOperation.replace);
    _root = _root.copyWith(
      children: [...newRoot],
    );
    logger.writeLog(
      level: LogLevel.info,
      message: 'Root of the project was re-setted. [Root => ${_root.length}]',
    );
    notifyListeners();
  }
  ...
  TreeNode? get visualSelection => _currentSelectedNode;

  @override
  TreeNode? backChild(Node node, [int? indexNode]) {
    verifyState();
    final isRootLevel = node.level == -1 && node.id == _root.node.id;
    if (isRootLevel && indexNode != null) {
      if (indexNode == 0) return null;
      return _root.elementAt(indexNode - 1);
    } else {
      for (int i = 0; i < _root.length; i++) {
        final treeNode = _root.elementAt(i);
        if (treeNode.node.id == node.id) {
          if (i == 0) return null;
          return _root.elementAt(i - 1);
  ...
  void clearChildrenByNode(String nodeId) {
    ...
    for (int index = 0; index < _root.length; index++) {
      final node = _root.elementAt(index);
      if (node is CompositeTreeNode && node.node.id == nodeId) {
        logger.writeLog(
          level: LogLevel.info,
          message:
              'Node ${node.id} was cleared. [Node(Level: ${node.level}) => ${node.length}]',
        );
        _root.addNewChange(
        ...
        _root[index] = node.copyWith(children: []);
        break;
  ...
  void clearRoot() {
    verifyState();
    _root.addNewChange([], TreeOperation.clearRoot);
    _root = _root.copyWith(children: []);
    logger.writeLog(
      level: LogLevel.info,
      message: 'Root of the project was cleared. [Root => ${_root.length}]',
    );
    notifyListeners();
  }
  ...
  TreeNode? getNodeWhere(bool Function(TreeNode node) predicate) {
    ...
    searchNodeWhere(_root.children);
  ...
  List<TreeNode>? getAllNodesInComposite(String nodeId) {
    ...
    for (var treenode in _root.children) {
  ...
  @override
  CompositeTreeNode<TreeNode>? getCompositeNode(String nodeId,
      {CompositeTreeNode<TreeNode>? compositeNode}) {
    verifyState();
  ...
  TreeNode? getNodeById(String nodeId) {
    verifyState();
    for (var node in _root.children) {
      if (node.node.id == nodeId) {
        return node;
      } else if (node is CompositeTreeNode && node.isNotEmpty) {
        final targetNode =
            searchChild(node, Node.base(nodeId), SearchStrategy.target);
    ...
    logger.writeLog(
      level: LogLevel.info,
      message:
          'Was not founded Node(id: $nodeId) or not exist in the current state of the tree',
    );
    throw InvalidNodeId(
        message:
            'The id: $nodeId is not valid or not exist currently into the tree');
  }
  ...
  void insertAbove(TreeNode node, String childBelowId,
    ...
    if (removeIfNeeded) {
      ...
      removeWhere((element) => element.node.id == node.node.id,
          verifyDuplicates: true);
    }
    for (int i = 0; i < _root.length; i++) {
      final belowNode = _root.elementAt(i);
      if (belowNode.node.id == childBelowId) {
        final beforeIndex = (i - 1) == -1 ? 0 : i;
        if (node is CompositeTreeNode) {
          node.formatChildLevels();
        }
        _root.addNewChange([node], TreeOperation.insertAbove, null, _root);
        _root.insert(
            beforeIndex,
            node.copyWith(
                nodeParent: _root.id,
                node: node.node.copyWith(level: belowNode.level)));
        break;
  ...
  bool insertAboveNodeInSubComposite(
      CompositeTreeNode composite, TreeNode node, String target) {
    for (int i = 0; i < composite.length; i++) {
      ...
      if (belowNode.node.id == target) {
        ...
        _root.addNewChange([node], TreeOperation.insertAbove, null, composite);
        composite.insert(
            i,
            node.copyWith(
                nodeParent: composite.id,
                node: node.node.copyWith(level: belowNode.level)));
        return true;
      } else if (belowNode is CompositeTreeNode && belowNode.isNotEmpty) {
    ...
    for (int i = 0; i < _root.length; i++) {
      ...
      if (belowNode.node.id == childBelowId) {
        ...
        _root.insert(
            beforeIndex,
            node.copyWith(
                nodeParent: _root.id, node: node.node.copyWith(level: 0)));
        break;
      } else if (belowNode is CompositeTreeNode && belowNode.isNotEmpty) {
        final inserted =
            insertAboveNodeInSubComposite(belowNode, node, childBelowId);
  ...
  @override
  void insertAt(TreeNode node, String nodeTargetId,
      {bool removeIfNeeded = false}) {
    logger.writeLog(
      level: LogLevel.info,
      message:
          'Should insert the node type ${node.runtimeType}(id: ${node.id}). [Root => ${_root.level}]',
    );
    if (removeIfNeeded) {
      removeAt(node.id, verifyDuplicates: true);
    }
    final existInRoot = !_root.existInRoot(node.id);
    if (_root.id == nodeTargetId && existInRoot) {
      if (node is CompositeTreeNode && node.isNotEmpty) {
        node.formatChildLevels(null, 0);
      }
      _root.add(node.copyWith(
          nodeParent: _root.id, node: node.node.copyWith(level: 0)));
      notifyListeners();
    } else {
      for (int i = 0; i < _root.length; i++) {
        final rootNode = _root.elementAt(i);
        if (rootNode.id == nodeTargetId) {
          ...
          if (!rootNode.existInRoot(node.id)) {
            ...
            rootNode.add(node.copyWith(
                nodeParent: rootNode.id,
                node: node.node.copyWith(level: rootNode.level + 1)));
            _root[i] = rootNode.copyWith(isExpanded: true);
            notifyListeners();
            return;
          } else {
          ...
          final inserted =
              insertNodeInSubComposite(rootNode, node, nodeTargetId);
          if (inserted) {
            notifyListeners();
            return;
          }
    ...
    for (int i = 0; i < compositeNode.length; i++) {
      ...
      if (treeNode is CompositeTreeNode) {
        if (treeNode.id == parentId) {
          ...
          treeNode.add(node.copyWith(
              node: node.node.copyWith(level: treeNode.level + 1),
              nodeParent: treeNode.id));
          return true;
  ...
  void insertAtRoot(TreeNode node, {bool removeIfNeeded = false}) {
    ...
    insertAt(node, _root.node.id);
    logger.writeLog(
      level: LogLevel.fine,
      message: 'Inserted node in Root level. [Root => ${_root.level}]',
    );
  }
  ...
  void insertAtWithCallback(String? parentId, TreeNode Function() callback,
    ...
    if (insertInRoot) {
      _root.add(node.copyWith(
          node: node.node.copyWith(level: 0), nodeParent: _root.id));
      notifyListeners();
      return;
    }
    for (int i = 0; i < _root.length; i++) {
      ...
      if (treeNode is CompositeTreeNode) {
        if (treeNode.id == parentId) {
          ...
          final inserted =
              insertAtInSubCompositeWithCallback(treeNode, callback, parentId);
          if (inserted) {
            _root[i] = treeNode.copyWith(isExpanded: true);
            break;
          }
        }
      }
    ...
    for (int i = 0; i < composite.length; i++) {
      ...
      if (compositeNode.node.id == target) {
        ...
        if (!composite.existInRoot(node.node.id)) {
          ...
          _root.addNewChange([node.copyWith(nodeParent: compositeNode.id)],
              TreeOperation.insert, null, compositeNode);
          compositeNode.add(node.copyWith(
              nodeParent: compositeNode.id,
              node: node.node.copyWith(level: compositeNode.level + 1)));
          composite[i] = compositeNode.copyWith(isExpanded: true);
          return true;
        } else {
          return true;
        }
      } else if (compositeNode is CompositeTreeNode &&
  ...
  TreeNode? nextChild(Node node, [int? indexNode]) {
    ...
    final isRootLevel = node.level == -1 && node.id == _root.node.id;
    if (isRootLevel && indexNode != null) {
      if (indexNode + 1 >= _root.length) return null;
      return _root.elementAt(indexNode + 1);
    } else {
      for (int i = 0; i < _root.length; i++) {
        final treeNode = _root.elementAt(i);
        if (treeNode.node.id == node.id) {
          if (i + 1 >= _root.length) return null;
          return _root.elementAt(i + 1);
        } else if (treeNode is CompositeTreeNode && treeNode.isNotEmpty) {
  ...
  bool openUntilNode(TreeNode targetNode, {bool openTargetIfNeeded = false}) {
    ...
    Future<bool> openWhen(List<TreeNode> tree,
  ...
  void removeAllInSelection() {
    for (int i = 0; i < _selection.length; i++) {
      final node = _selection.elementAt(i);
  ...
  @override
  TreeNode? removeAt(String nodeId,
      {bool ignoreRoot = false, bool verifyDuplicates = false}) {
    if (existInRoot(nodeId)) {
      _root.removeWhere((element) => element.id == nodeId);
      notifyListeners();
      logger.writeLog(
        level: LogLevel.fine,
        message: 'Removed in Root level. [Root change => ${_root.level}]',
      );
    ...
    for (int i = 0; i < _root.length; i++) {
      ...
      if (node.id == nodeId && !ignoreRoot) {
        ...
        logger.writeLog(
          level: LogLevel.fine,
          message:
              'Removed ${node.runtimeType}(id: ${node.id}) in Root level. [Root change => ${_root.level}]',
        );
        notifyListeners();
        return node;
      } else if (node is CompositeTreeNode && node.isNotEmpty) {
        ...
        if (isFounded) {
          ...
          logger.writeLog(
            level: LogLevel.fine,
            message:
                'Removed sub ${node.runtimeType}(id: ${node.id}) node. [Root change => ${_root.level}]',
          );
          notifyListeners();
          return node;
        }
  ...
  bool removeChild(
    ...
    for (int i = 0; i < compositeNode.length; i++) {
      ...
      if (node.node.id == target?.id || predicate?.call(node) == true) {
        _root.addNewChange([node], TreeOperation.delete, null, compositeNode);
        compositeNode.remove(node);
        if (!verifyDuplicates) return true;
      } else if (node is CompositeTreeNode && node.isNotEmpty) {
        final foundedNode =
            removeChild(node, target, predicate, verifyDuplicates);
        if (foundedNode) return true;
      }
    }
    return false;
  ...
  SelectableTreeNode? removeFromSelection(String nodeId) {
    if (_selection.checkExistence(nodeId)) {
      final node = removeWhere((element) => element.node.id == nodeId)
          as SelectableTreeNode?;
      if (node == null) {
        throw InvalidNodeId(
            message: 'The node: $nodeId is invalid or not exist into the tree');
      }
      _selection.removeWhere((element) => element.node.id == nodeId);
      notifyListeners();
      return node;
    }
    return null;
  ...
  void setNewIdToRoot(String id) {
    ...
    assert(id.replaceAll(RegExp(r'\p{Z}', unicode: true), '').isNotEmpty,
        'The id passed cannot be empty');
    final lastNodeState = _root.node;
    final newRootState = _root.clone();
    newRootState.updateInternalNodesByParentId(id);
    _root.addNewChange(
        [...newRootState.children], TreeOperation.update, lastNodeState);
    _root = _root.copyWith(
      node: _root.node.copyWith(
        id: id,
      ),
    );
    _root.updateInternalNodesByParentId(id);
    notifyListeners();
  }
  ...
  void setVisualSelection(TreeNode node) {
    verifyState();
    if (_currentSelectedNode?.node.id == node.node.id) return;
  ...
  bool updateInSelectionNodeAt(SelectableTreeNode node) {
    if (_selection.checkExistence(node.node.id)) {
      final indexOf =
          _selection.indexWhere((element) => element.node.id == node.node.id);
      if (indexOf == -1) return false;
      _selection[indexOf] = node;
      notifyListeners();
    }
    return false;
  }
  ...
  @protected
  bool updateSubNodes(
      List<TreeNode> children, TreeNode targetNode, String nodeId) {
    bool updated = false;
...
class _Root extends CompositeTreeNode<TreeNode> {
  ...
  void addNewChange(List<TreeNode> nodes, TreeOperation op,
      [Node? oldNodeState, TreeNode? changedNode]) {
